...
description: Generates or updates a comprehensive markdown data dictionary from any database, auto-detecting schema source and DB engine. Includes live statistics, sample rows, null rates, Mermaid ER diagram, and quick-reference table. Use when documenting a new database, onboarding team members, or refreshing schema stats (--update).
user-invocable: true
---
...
When `--update` is passed and the output file exists, read [UPDATE_MODE.md](UPDATE_MODE.md) for the full merge procedure (parse existing file → diff schema → merge → print summary) instead of Step 5.
