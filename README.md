...
| [grill-science-with-docs](grill-science-with-docs/) | `/grill-science-with-docs` | Three-phase interrogation for scientific analysis you did not originate — Phase 0 grills the **question** (what decision hangs on it, what the proxy question hides, has the field already answered it, cheapest test that could kill it), Phase 1 grills the **data** (source methods, complete structure enumeration, units, denominators), each behind a hard gate; Phase 2 runs a pre-specified analysis with rendered-and-inspected figures, a correction log, and an explicit statement of what the data cannot settle |
