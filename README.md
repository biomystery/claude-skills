...
| [note-to-moc](note-to-moc/) | `/note-to-moc` | Refactor an overgrown Obsidian note into a MOC hub + spoke notes in subfolders — slices by exact line range so verbatim content is never retyped, moves via the Obsidian CLI so links rewrite themselves, recurses into any spoke that is still overgrown so it becomes a sub-hub, verifies zero content loss, zero broken links and zero stranded notes |
| [grill-science-with-docs](grill-science-with-docs/) | `/grill-science-with-docs` | A relentless interview that sharpens a scientific claim and **closes the loop** on it, producing durable records as it goes — **FRAME** (state it so it could be false, name the decision, separate the real question from the proxy, check prior art) → **GROUND** (read the primary source, enumerate completely before asserting absence, fix the unit, don't inherit the source's self-description) → **TEST** (pre-specify the interpretation, interrogate every denominator, inspect your own figures) → **CLOSE** (settled vs ruled out vs open, corrections kept visible, what would change the conclusion). Gated at every phase |
